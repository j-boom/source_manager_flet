...
class FieldEditorCard(ft.Card):
    ...
    def __init__(self, field_data: Dict[str, Any], config_type: str, colors: ft.ColorScheme, on_delete: Callable, field_types: List[str], autofocus: bool = False):
    ...
    def _build_card_content(self) -> ft.Container:
        ...
        self.form_controls['name'] = ft.TextField(
            ...
            read_only=bool(self.field_data.get("name")), # Make name read-only if it already exists
            expand=2
        )
        ...
        self.form_controls['required'] = ft.Switch(label="Required", value=self.field_data.get("required", False))
        self.form_controls['is_filterable'] = ft.Switch(label="Filterable", value=self.field_data.get("is_filterable", False), visible=(self.config_type == 'source'))
        self.form_controls['is_title_part'] = ft.Switch(label="Part of Title", value=self.field_data.get("is_title_part", False), visible=(self.config_type == 'source'))
        self.form_controls['show_in_editor'] = ft.Switch(label="Show in Editor", value=self.field_data.get("show_in_editor", True))

        storage_scope_control = None
        if self.config_type == 'source':
            self.form_controls['storage_scope'] = ft.Dropdown(
                label="Storage Scope",
                options=[ft.dropdown.Option("core", "Master Record"), ft.dropdown.Option("link", "Project Link")],
            ...
            controls=[
                ft.Row(
                    [
                        ...
                        ft.IconButton(icon=ft.icons.DELETE_OUTLINE, on_click=lambda e: self.on_delete(self), tooltip="Delete Field", icon_color=self.colors.error)
                    ],
                    spacing=10
                ),
        ...
        return ft.Container(content, padding=15, border=ft.border.all(1, self.colors.outline_variant), border_radius=8)
