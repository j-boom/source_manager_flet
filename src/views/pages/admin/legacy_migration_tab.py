...
from utils import generate_source_title, create_validated_field
...
class LegacyMigrationTab(ft.Column):
    ...
    def update_view(self):
        ...
        if legacy_source:
            ...
            option_keys = [opt.key for opt in self.source_type_dropdown.options]
            if parsed_source_type and parsed_source_type in option_keys:
        ...
        for s_config_dict in fields_to_create:
            if s_config_dict.get("show_in_editor", True):
                ...
                compatible_config = SourceCreationDialog._CompatibleFieldConfig.from_source_field_config(
                    SourceFieldConfig(**s_config_dict)
                )
                
                initial_value = pre_filled_data.get(compatible_config.name, "")
                widget = create_validated_field(compatible_config, initial_value=str(initial_value))
