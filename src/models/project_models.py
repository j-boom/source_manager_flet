    ...
    def from_dict(
        ...
        if "project_metadata" in data:
            ...
            for source_data in data.get("sources", []):
                sources.append(
                    ProjectSourceLink(
                        ...
                        notes=source_data.get(
                            "usage_notes", ""
                        ),  # Read from 'usage_notes' key
                        declassify=source_data.get(
                            "declassify", ""
                        ),  # Read from 'declassify' key
                    )
                )
            ...
            if "project_type" not in data:
                ...
                if " - STD - " in str(file_path_str):
                    data["project_type"] = "STD"
                elif " - COM - " in str(file_path_str):
                    data["project_type"] = "COM"
                elif " - FCR - " in str(file_path_str):
                    data["project_type"] = "FCR"
                elif " - GSC - " in str(file_path_str):
                    data["project_type"] = "GSC"
                elif " - CCR - " in str(file_path_str):
                    data["project_type"] = "CCR"
                elif " - CRS - " in str(file_path_str):
            ...
            if "file_path" not in data:
                data["file_path"] = str(file_path or Path.cwd())
            ...
            if "project_title" not in data:
                data["project_title"] = (
                    file_path.stem if file_path else "Legacy Project"
                )
